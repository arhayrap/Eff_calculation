#include "../interface/TrigScaleFactors_OnMC.h"
...
double TrigScaleFactors_OnMC::calculateMuonSVProbability(const pat::Muon& muon, const std::vector<reco::Vertex>& vertices) {
...
double TrigScaleFactors_OnMC::calculateBackpointingAngle(const reco::Vertex& primaryVertex, const reco::VertexCompositePtrCandidate& secondaryVertex, const pat::Muon& muon) {
...
double TrigScaleFactors_OnMC::DeltaR(double eta1, double eta2, double phi1, double phi2) {
    return sqrt(pow(eta1 - eta2, 2) + pow(MyFunc::phi_dist(phi1, phi2), 2));
}

///////////////////////////////////////////////////////////////////

TrigScaleFactors_OnMC::TrigScaleFactors_OnMC(const edm::ParameterSet& iConfig):
...
TrigScaleFactors_OnMC::~TrigScaleFactors_OnMC(){}

// Function to calculate the backpointing angle


//void TrigScaleFactors_OnMC::endRun(const edm::Run& iRun, const edm::EventSetup& iSetup) {

//}

void TrigScaleFactors_OnMC::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
...
// Generator weights *********************************************************
  ...
  double Gweight=genEvtInfo->weight();
...
// Offline Muons ***************************************************************
  ...
  for (auto mu = recmuons->begin(); mu != recmuons->end(); mu++)
   {
    ...
    if (mu->isTightMuon(PV) && !tight_id) { // The tag muon wasn't selected and is here.
        ...
        if (mu->isGlobalMuon() && mu->muonBestTrack().isNonnull()) {
            ...
            probSV[N_Off_Mu] = TrigScaleFactors_OnMC::calculateMuonSVProbability(*mu, *vertices);
            if (secondaryVerticesHandle.isValid() && !(secondaryVerticesHandle->empty())) {
                mu_PV_theta[N_Off_Mu] = TrigScaleFactors_OnMC::calculateBackpointingAngle(PV, SV, *mu);
   ...
   for(size_t i = 0; i < genParticles->size(); ++ i) {
     ...
     if (abs(id) == 13 && mom->pdgId() == 443) {
       double dr_tag_0 = TrigScaleFactors_OnMC::DeltaR(mueta[0], eta, muphi[0], phi);
       double dr_probe_0 = TrigScaleFactors_OnMC::DeltaR(mueta[1], eta, muphi[1], phi);
       if (dr_tag > dr_tag_0) {
           dr_tag = dr_tag_0;
       }
   ...
   */
   ...
   for (size_t d = 0; d < SV.numberOfDaughters(); d++) {
       ...
       double dr_tag_0 = TrigScaleFactors_OnMC::DeltaR(mueta[0], track->eta(), muphi[0], track->phi());
       double dr_probe_0 = TrigScaleFactors_OnMC::DeltaR(mueta[1], track->eta(), muphi[1], track->phi());
       if (dr_tag > dr_tag_0) {
           dr_tag = dr_tag_0;
       }
       if (dr_probe > dr_probe_0) {
...
// *************************************** Ntuples ******************************************
         _Tag_Candidate   -> Fill (mupt[0],mue[0],mueta[0],muphi[0],mucharge[0],mudxy[0],mudxyErr[0],mudz[0],muLooseID[0],muTightID[0],mu_isGlobal[0],Gweight,Tnpv);
...
void TrigScaleFactors_OnMC::beginJob(){
}

    TH1F *TrigScaleFactors_OnMC::book1DHistogram(TFileDirectory & fDir, const std::string & fName, const std::string & fTitle,
    ...
    TProfile *TrigScaleFactors_OnMC::bookProfileHistogram(TFileDirectory & fDir, const std::string & fName, const std::string & fTitle,
    ...
    TH2F *TrigScaleFactors_OnMC::book2DHistogram(TFileDirectory & fDir, const std::string & fName, const std::string & fTitle,
   ...
   void TrigScaleFactors_OnMC::bookHistograms() {
...
void TrigScaleFactors_OnMC::endJob() {
}

DEFINE_FWK_MODULE(TrigScaleFactors_OnMC);
